...
import type { FileCategory, UniversalFileDescriptor, FileAttributes, InputSource, DescriptorWarning } from "../domain/descriptors";
...
const EXT_CATEGORY: Record<string, FileCategory> = {
  ...
  opus: "audio", aiff: "audio", aac: "audio", wma: "audio", alac: "audio",
  // Video
  mp4: "video", webm: "video", mkv: "video", avi: "video", mov: "video",
  wmv: "video", flv: "video", m4v: "video", ts: "video", mts: "video",
  // Image
  jpg: "image", jpeg: "image", png: "image", gif: "image", webp: "image",
  avif: "image", tiff: "image", tif: "image", bmp: "image", ico: "image",
  svg: "image", heic: "image", heif: "image",
  // Document/markup
  md: "plain-text", markdown: "plain-text", txt: "plain-text", rtf: "document",
  html: "plain-text", htm: "plain-text",
  ...
  bz2: "archive", xz: "archive", rar: "archive", wim: "archive", lz4: "archive",
  // Structured data
  json: "structured-data", jsonl: "structured-data",
  yaml: "structured-data", yml: "structured-data",
  toml: "structured-data", xml: "structured-data",
};
...
const MAGIC_TABLE: MagicEntry[] = [
  // Images
  { offset: 0, bytes: Buffer.from([0xff, 0xd8, 0xff]), mime: "image/jpeg", format: "jpeg" },
  { offset: 0, bytes: Buffer.from([0x89, 0x50, 0x4e, 0x47]), mime: "image/png", format: "png" },
  { offset: 0, bytes: Buffer.from([0x47, 0x49, 0x46]), mime: "image/gif", format: "gif" },
  { offset: 0, bytes: Buffer.from([0x52, 0x49, 0x46, 0x46]), mime: "image/webp", format: "webp" }, // RIFF
  { offset: 8, bytes: Buffer.from([0x57, 0x45, 0x42, 0x50]), mime: "image/webp", format: "webp" },
  { offset: 0, bytes: Buffer.from([0x49, 0x49, 0x2a, 0x00]), mime: "image/tiff", format: "tiff" },
  { offset: 0, bytes: Buffer.from([0x4d, 0x4d, 0x00, 0x2a]), mime: "image/tiff", format: "tiff" },
  // PDF
  { offset: 0, bytes: Buffer.from([0x25, 0x50, 0x44, 0x46]), mime: "application/pdf", format: "pdf" },
  // ZIP-based (DOCX, XLSX, EPUB, etc.) — detected by structure after
  { offset: 0, bytes: Buffer.from([0x50, 0x4b, 0x03, 0x04]), mime: "application/zip", format: "zip" },
  { offset: 0, bytes: Buffer.from([0x50, 0x4b, 0x05, 0x06]), mime: "application/zip", format: "zip" },
  // 7-Zip
  { offset: 0, bytes: Buffer.from([0x37, 0x7a, 0xbc, 0xaf, 0x27, 0x1c]), mime: "application/x-7z-compressed", format: "7z" },
  // gzip
  { offset: 0, bytes: Buffer.from([0x1f, 0x8b]), mime: "application/gzip", format: "gz" },
  // bzip2
  { offset: 0, bytes: Buffer.from([0x42, 0x5a, 0x68]), mime: "application/x-bzip2", format: "bz2" },
  // xz
  { offset: 0, bytes: Buffer.from([0xfd, 0x37, 0x7a, 0x58, 0x5a, 0x00]), mime: "application/x-xz", format: "xz" },
  // OGG
  { offset: 0, bytes: Buffer.from([0x4f, 0x67, 0x67, 0x53]), mime: "audio/ogg", format: "ogg" },
  // FLAC
  { offset: 0, bytes: Buffer.from([0x66, 0x4c, 0x61, 0x43]), mime: "audio/flac", format: "flac" },
  // MP3 (ID3)
  { offset: 0, bytes: Buffer.from([0x49, 0x44, 0x33]), mime: "audio/mpeg", format: "mp3" },
  // WAV (RIFF....WAVE)
  { offset: 0, bytes: Buffer.from([0x52, 0x49, 0x46, 0x46]), mime: "audio/wav", format: "wav" },
  // Windows PE (executables — should be rejected)
  { offset: 0, bytes: Buffer.from([0x4d, 0x5a]), mime: "application/x-dosexec", format: "exe" },
];

async function detectByMagic(filePath: string): Promise<{ mime: string; format: string } | null> {
  const fd = fs.openSync(filePath, "r");
  try {
    const headerBuf = Buffer.alloc(16);
    fs.readSync(fd, headerBuf, 0, 16, 0);
    for (const entry of MAGIC_TABLE) {
      const slice = headerBuf.slice(entry.offset, entry.offset + entry.bytes.length);
      if (slice.equals(entry.bytes)) {
        ...
        if (entry.format === "webp" || entry.format === "wav") {
          ...
          if (typeBytes === "WEBP") return { mime: "image/webp", format: "webp" };
...
function probeZipContents(filePath: string, ext: string | null): { mime: string; format: string } {
  try {
    ...
    if (str.includes("word/document.xml")) return { mime: "application/vnd.openxmlformats-officedocument.wordprocessingml.document", format: "docx" };
    if (str.includes("xl/workbook.xml")) return { mime: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", format: "xlsx" };
    if (str.includes("ppt/presentation.xml")) return { mime: "application/vnd.openxmlformats-officedocument.presentationml.presentation", format: "pptx" };
    if (str.includes("META-INF/container.xml") && str.includes("epub")) return { mime: "application/epub+zip", format: "epub" };
    if (str.includes("content.xml") && str.includes("manifest.xml")) {
      if (str.includes("Writer")) return { mime: "application/vnd.oasis.opendocument.text", format: "odt" };
      if (str.includes("Calc")) return { mime: "application/vnd.oasis.opendocument.spreadsheet", format: "ods" };
      if (str.includes("Impress")) return { mime: "application/vnd.oasis.opendocument.presentation", format: "odp" };
    }
...
function probeTextStructure(filePath: string): { mime: string; format: string } | null {
  try {
    ...
    if ((firstLine.match(/,/g) ?? []).length >= 2) return { mime: "text/csv", format: "csv" };
    if ((firstLine.match(/\t/g) ?? []).length >= 1) return { mime: "text/tab-separated-values", format: "tsv" };
...
const DANGEROUS_EXTENSIONS = new Set([
  "exe", "dll", "msi", "bat", "cmd", "com", "ps1", "vbs", "js", "jar",
  "scr", "pif", "hta", "wsf", "reg", "lnk",
]);
...
export async function detectFile(filePath: string): Promise<DetectionResult> {
  ...
  if (magic) { mime = magic.mime; format = magic.format; }
  ...
  if (!mime) {
    ...
    if (textProbe) { mime = textProbe.mime; format = textProbe.format; }
  }
  ...
  if (mime) {
    ...
    warnings.push({ code: "MIME_FROM_EXTENSION", message: "Tipo MIME derivado solo de la extensión", severity: "info" });
  }

  // 6. Warn on executable
  if (mime && DANGEROUS_MIMES.has(mime)) {
    warnings.push({ code: "EXECUTABLE_REJECTED", message: "El archivo es un ejecutable y no puede convertirse", severity: "danger" });
    category = "unknown";
  }
  if (ext && DANGEROUS_EXTENSIONS.has(ext)) {
    warnings.push({ code: "DANGEROUS_EXTENSION", message: `Extensión peligrosa detectada: .${ext}`, severity: "danger" });
  }
  ...
  return { category, detectedMimeType: mime, detectedFormat: format, attributes, warnings };
}

function buildAttributes(category: FileCategory, format: string | null): FileAttributes {
  switch (category) {
    case "audio":
    case "video":
      return { kind: "media", durationSeconds: null, bitrate: null, hasAudio: category === "audio", hasVideo: category === "video", hasSubtitles: false, audioCodec: null, videoCodec: null, width: null, height: null, fps: null };
    case "image":
      return { kind: "image", width: 0, height: 0, channels: null, hasAlpha: false, format: format, colorSpace: null, animated: false, frames: 1, densityPpi: null, iccProfile: null };
    case "document":
      return { kind: "document", pageCount: null, wordCount: null, hasMacros: false, hasEmbeddedMedia: false, encoding: null, language: null };
    case "spreadsheet":
      return { kind: "spreadsheet", sheetCount: null, rowCount: null, columnCount: null, hasMacros: false, hasFormulas: false, hasCharts: false };
    case "presentation":
      return { kind: "presentation", slideCount: null, hasMacros: false, hasEmbeddedMedia: false, hasAnimations: false };
    case "pdf":
      return { kind: "pdf", pageCount: null, isEncrypted: false, isLinearized: false, pdfVersion: null, hasAnnotations: false, hasForms: false, hasEmbeddedFiles: false };
    case "ebook":
      return { kind: "ebook", hasDrm: false, pageCount: null, title: null, author: null, language: null, publisher: null, ebookFormat: format };
    case "archive":
      return { kind: "archive", entryCount: null, uncompressedBytes: null, expansionRatio: null, isEncrypted: false, maxDepth: null, hasDangerousPaths: false, archiveFormat: format };
    case "structured-data":
      return { kind: "structured-data", format: format ?? "unknown", rowCount: null, columnCount: null, encoding: "utf-8", isTabular: ["csv", "tsv"].includes(format ?? ""), hasNestedStructures: false, hasXmlEntities: false };
    case "plain-text":
      return { kind: "text", encoding: "utf-8", lineCount: null, format: format ?? "txt" };
    default:
      return { kind: "unknown" };
  }
}
...
export async function buildDescriptor(
  ...
  const originalName = source.kind === "local-upload" ? source.originalName : path.basename(filePath);
