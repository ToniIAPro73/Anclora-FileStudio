# Anclora FileStudio Agent Guide
...
La arquitectura estable del ecosistema se define fuera de este repo, en la Boveda
Anclora. No asumir infraestructura compartida entre productos. Validar siempre
hosting, backend, base de datos, auth, variables y ramas.
<!-- ANCLORA-ECOSYSTEM-CONTEXT-END -->

<!-- ANCLORA-SDD-STANDARDS-START -->
## Metodologia SDD - Estandar Unificado Anclora

Todo desarrollo en este repo sigue la metodologia SDD unificada del ecosistema
Anclora.

### Flujo de trabajo Git

- Rama base de desarrollo: `development`.
- Los agentes crean ramas desde `development`: `feat/<agente>-<descripcion>`,
  `fix/<agente>-<descripcion>` o `chore/<agente>-<descripcion>`.
- Las ramas se mergean de vuelta a `development` via PR.
- Promocion manual: `development -> staging -> production -> main`.
- No commitear directamente en `main`, `staging` ni `production`.

### Ciclo de cambios

Toda feature o fix sigue este flujo antes de escribir codigo:

- Crear spec: `sdd/features/<feature-id>/<feature-id>-spec-v1.md`.
- Crear plan: `sdd/features/<feature-id>/<feature-id>-plan-v1.md` si el cambio
  es complejo.
- Crear tasks: `sdd/features/<feature-id>/<feature-id>-tasks-v1.md`.
- Implementar tarea a tarea, con tests primero cuando aplique.
- Validar contra criterios de aceptacion de la spec.
- Abrir PR contra `development`, con referencia a la spec.

### Reglas obligatorias

- No spec, no code para nuevas features.
- Tests primero para cambios de comportamiento.
- Mantener el modo browser/Vercel separado del modo Desktop/portable.
- Los procesos externos se ejecutan con `shell: false`.
- Cambios de UX visible deben revisar copy, accesibilidad e i18n.
- Una spec cerrada no se edita; los cambios generan una version nueva.
